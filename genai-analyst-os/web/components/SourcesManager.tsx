...
// The list of URLs/RSS feeds Signal crawls into your Feed — shared between
// the standalone /sources page (kept for direct links) and the Settings
// "Source Feeds" tab, so the add/remove logic lives in exactly one place.
export default function SourcesManager({ showChecklist = true }: { showChecklist?: boolean }) {
  ...
  if (authLoading) {
    return <div className="h-48 animate-pulse rounded-3xl bg-zinc-100 dark:bg-zinc-800" />
  }

  if (!session || !userId) {
    return (
      <div className="rounded-3xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 p-8">
        <p className="text-sm text-zinc-500 dark:text-zinc-400">Sign in first to manage your private source list. Sources are account-scoped and can be removed at any time.</p>
      </div>
    )
  }
  ...
  return (
    <div>
      ...
      {showChecklist && setupStatus && !setupStatus.checklistComplete && (
        <div className="mb-8">
