...
export default function FeedPage() {
  ...
  const fetchNarrative = useCallback(async () => {
    ...
    try {
      ...
      else setNarrativeError(json.error ?? 'No articles found for the last 7 days')
  ...
  useEffect(() => {
    ...
    if (activeTab === 'weekly' && weeklyItems.length === 0) { fetchWeekly(); fetchNarrative() }
  }, [activeTab, newsItems.length, weeklyItems.length, fetchNews, fetchWeekly, fetchNarrative])
        ...
        <div>
          <div className="flex items-center justify-between mb-5">
            <div>
              ...
              <p className="text-xs text-zinc-400 mt-0.5">Narrative generated by Claude Sonnet from your top articles</p>
            </div>
            <div className="flex items-center gap-2">
              <div className="flex gap-1 bg-zinc-100 dark:bg-zinc-800 p-0.5 rounded-lg">
              ...
              {weeklyView === 'narrative' && (
                <button onClick={fetchNarrative} disabled={narrativeLoading}
                  className="text-xs text-violet-600 dark:text-violet-400 px-3 py-1.5 bg-violet-50 dark:bg-violet-950/30 rounded-lg border border-violet-200 dark:border-violet-800 hover:bg-violet-100 transition-colors font-medium disabled:opacity-50">
                  {narrativeLoading ? 'Writing…' : '↺ Regenerate'}
                </button>
              )}
            </div>
          </div>
            ...
            <div>
              {narrativeLoading ? (
                ...
                <div className="text-center py-16 text-zinc-400">
                  ...
                  <p className="text-sm mt-2">Run the pipeline first to build your article base, then come back here.</p>
                </div>
              ) : narrative ? (
